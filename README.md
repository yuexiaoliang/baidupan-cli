...
上传前会通过百度 `locateupload` 接口获取可用的 HTTPS 上传节点池，并对所有节点并行执行最长 5 秒的无请求体探测，优先选择响应最快的可达节点；不可达节点排在队尾，动态节点获取失败时回退到 `https://d.pcs.baidu.com`。每个分块最多尝试 3 次，遇到连接超时或服务端错误时会重新发现并切换节点。重试会重新创建上传请求，避免复用已经消费的 multipart 数据流。
...
- 确认能正常访问百度网盘 API
- 使用 `--verbose` 查看各节点测速结果和最终上传节点优先级
- 分块上传失败时会重新发现和切换节点，重新创建请求并最多尝试 3 次
